    ...
    impl CpalAudioOutput {
        pub fn try_open(spec: SignalSpec, duration: Duration) -> Result<Box<dyn AudioOutput>> {
            ...
            let device = match host.default_output_device() {
                Some(device) => device,
                _ => {
                    tracing::error!("failed to get default audio output device");
            ...
            let config = match device.default_output_config() {
                Ok(config) => config,
                Err(err) => {
                    tracing::error!("failed to get default audio output device config: {}", err);
                    return Err(AudioOutputError::OpenStreamError);
                }
            };
    ...
    impl<T: AudioOutputSample> CpalAudioOutputImpl<T> {
        pub fn try_open(spec: SignalSpec, duration: Duration, device: &cpal::Device) -> Result<Box<dyn AudioOutput>> {
            ...
            let stream_result = device.build_output_stream(
                ...
                move |err| tracing::error!("audio output error: {}", err),
                None,
            );

            if let Err(err) = stream_result {
                tracing::error!("audio output stream open error: {}", err);
            ...
            if let Err(err) = stream.play() {
                tracing::error!("audio output stream play error: {}", err);
